    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :imgUrlList="imgUrlList" />
          <!-- 小图列表 -->
          <ImageList :imgUrlList="imgUrlList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="itemArr in spuSaleAttrList" :key="itemArr.id">
                <dt class="title">{{ itemArr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  v-for="item in itemArr.spuSaleAttrValueList"
                  :key="item.id"
                  :class="{ active: item.isChecked == 1 }"
                  @click="checkId(item, itemArr.spuSaleAttrValueList)"
                  ref="btns"
                >
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据 因为可能返回undefind所以要先处理一下
    imgUrlList() {
      return this.skuInfo.skuImageList || [{}];
    },
  },
  methods: {
    //这里点击售卖属性显示高亮 使用排他思想 el是当前点击的这个的节点属性 arr是当前点击节点里面的数组数据
    checkId(el, arr) {
      console.log(el);
      console.log(arr);
      //遍历全部的售卖属性值的isChecked 时遍历的属性值全为0 这样取消高亮
      arr.forEach((item) => {
        console.log(item.isChecked);
        item.isChecked = 0;
      });
      //当前点击节点的isChecked为1 显示高亮
      el.isChecked = 1;
    },
    //表单元素修改产品的个数
    changeSkuNum(e) {
    ...
    //点击加入购物车按钮跳转到添加购物车成功界面并传递参数和发送请求
    async addShopCar() {
      //这里派发actions如果带多个参数要以对象的形式进行传参
      //这是派发也就是执行addOrupDateShopCart这个函数的回调 因为async返回的是一个promise对象
      //所以这里要用await和async
      ...
      try {
        ...
        //如果成功那么进行路由的跳转
        // this.$bus.$emit("addShopCateInfo", this.skuInfo);
        this.$router.push({
          name: "AddCartSuccess",
          query: {
            skuNum: this.skuNum,
        ...
        //使用浏览器本地存储传递skuInfo
        //JSON.stringify(this.skuInfo)浏览器本地存储需要把对象转换成字符串才能使用
